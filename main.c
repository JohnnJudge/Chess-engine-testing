...
{
    //listen to GUI/user input every 2047 nodes
    if((nodes & 2047) == 0)
    {
        communicate();
    }
    ...
    for (int count = 0; count < move_list->count; count++)
    {
        ...
        legal_moves++;
            if(moves_searched == 0){
                ...
                {
                    if((moves_searched >= FullDepthMoves) && (depth >= ReductionLimit) && (in_check == 0 && get_move_capture(move_list->moves[count]) == 0 
...
void search_position(int depth)
{
    ...
    int alpha = -50000;
    int beta = 50000;
    for(int curr = 1; curr <= depth; curr++)
    {
        if(stopped == 1)
            break;
        follow_pv = 1;
        score = negamax(alpha, beta, curr);
        if((score <= alpha) || (score >= beta))
        {
            alpha = -50000;
            beta = 50000;
        ...
        alpha = score - 50;
        beta = score + 50;
        printf("info score cp %d depth %d nodes %ld pv ", score, curr, nodes);
...
void uci_loop()
{
    ...
    setbuf(stdout, NULL);
    char input[2000];
    printf("id name john chess engine\n");
    printf("id name john\n");
    printf("uciok\n");
    while (1)
    {
        ...
        {
            printf("id name johns chess engine\n");
            printf("id name john\n");
            printf("uciok\n");
        }
    }
